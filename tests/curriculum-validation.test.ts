  ...
  return {
    sources: [
      ...
      },
      {
        id: 'PM-2022-3B',
        type: 'TEXTBOOK',
        title: 'Primary Mathematics Student Book 3B',
        edition: '2022',
        isbn: '9789814911412',
      },
    ],
    curriculum: {
      id: 'SG-MATH-2021',
      name: 'Singapore Primary Mathematics',
      country: 'Singapore',
      version: '2021',
      sourceRefs: [{ sourceId: 'MOE-PM-2021-OCT-2025', locator: 'Primary 3' }],
    },
    nodes: [
      {
        id: 'P3',
      ...
      },
      {
        ...
        parentId: 'P3',
        sequence: 1,
        sourceRefs: [{ sourceId: 'MOE-PM-2021-OCT-2025', locator: 'P3 / Number and Algebra' }],
      },
      {
        id: 'P3-FRACTIONS',
        type: 'topic',
        ...
        parentId: 'P3-NUMBER',
        sequence: 4,
        sourceRefs: [{ sourceId: 'MOE-PM-2021-OCT-2025', locator: 'P3 / Number and Algebra / Fractions' }],
      },
    ],
    objectives: [makeObjective('P3-FRA-001')],
    representations: [
      {
        id: 'REP-FRACTION-STRIP',
        stage: 'PICTORIAL',
        name: 'Fraction strip',
        description: 'Equal-length strips partitioned into equal parts.',
        sourceRefs: [{ sourceId: 'MOE-PM-2021-OCT-2025', locator: 'Pedagogy / representations' }],
      },
    ],
    strategies: [
      {
        id: 'STRAT-DRAW-DIAGRAM',
        name: 'Draw a diagram',
        description: 'Represent quantities and relationships visually.',
        sourceRefs: [{ sourceId: 'MOE-PM-2021-OCT-2025', locator: 'Mathematical processes / heuristics' }],
      },
    ],
    misconceptions: [
      {
        id: 'MIS-FRA-DENOMINATOR-SIZE',
        name: 'Larger denominator means larger fraction',
        description: 'Treats the denominator as a whole-number magnitude rather than number of equal parts.',
        evidenceSignals: ['Claims 1/8 is greater than 1/4 because 8 is greater than 4.'],
        sourceRefs: [{ sourceId: 'MOE-PM-2021-OCT-2025', locator: 'P3 / Fractions; MathMagics teaching annotation' }],
      },
    ],
    textbookReferences: [
      {
        id: 'PM-2022-3B-CH7-7E',
        sourceId: 'PM-2022-3B',
        series: 'Primary Mathematics',
        edition: '2022',
        book: '3B',
        chapter: 'Chapter 7: Fractions',
        lesson: '7E: Equivalent Fractions',
...
function makeObjective(id: string, overrides: Partial<LearningObjective> = {}): LearningObjective {
  return {
    ...
    description: 'Recognise when two fractions represent the same value.',
    sequence: 1,
    sourceRefs: [{ sourceId: 'MOE-PM-2021-OCT-2025', locator: 'P3 / Number and Algebra / Fractions' }],
    prerequisiteIds: [],
    representationIds: [],
...
describe('curriculum domain types', () => {
  it('constructs a minimal valid dataset fixture', () => {
    ...
    expect(dataset.textbookReferences[0]?.sourceId).toBe('PM-2022-3B');
  });
});
...
describe('validateCurriculumDataset', () => {
  ...
  it('reports unknown objective annotation references', () => {
    const dataset = makeValidDataset();
    Object.assign(dataset.objectives[0]!, {
      prerequisiteIds: ['MISSING-OBJECTIVE'],
      representationIds: ['MISSING-REP'],
      strategyIds: ['MISSING-STRATEGY'],
      misconceptionIds: ['MISSING-MISCONCEPTION'],
    });
    const errors = validateCurriculumDataset(dataset);
  ...
  it('reports unknown provenance sources', () => {
    ...
    expect(validateCurriculumDataset(dataset)).toContain(
      'Curriculum SG-MATH-2021 references unknown source: UNKNOWN-SOURCE',
    );
  });

  it('reports broken textbook references', () => {
    const dataset = makeValidDataset();
  ...
  it('reports prerequisite cycles with involved objective IDs', () => {
    const dataset = makeValidDataset();
    dataset.objectives = [
      makeObjective('P3-FRA-A', { prerequisiteIds: ['P3-FRA-B'] }),
      makeObjective('P3-FRA-B', { prerequisiteIds: ['P3-FRA-A'], sequence: 2 }),
    ];

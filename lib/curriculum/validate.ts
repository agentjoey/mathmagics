...
function validateSourceRefs(
  ...
  for (const ref of refs) {
    if (!sourceIds.has(ref.sourceId)) {
      errors.push(`${owner} references unknown source: ${ref.sourceId}`);
    }
    if (!ref.locator.trim()) {
      errors.push(`${owner} has an empty source locator for: ${ref.sourceId}`);
    }
  }
  return errors;
}
...
function findPrerequisiteCycles(dataset: CurriculumDataset): string[] {
  ...
  const edges = new Map(
    dataset.objectives.map((objective) => [
      objective.id,
      objective.prerequisiteIds.filter((id) => objectiveIds.has(id)),
    ]),
  );
  ...
  function visit(id: string) {
    ...
    if (state.get(id) === 'visiting') {
      ...
      cycles.add(`Prerequisite cycle: ${path.join(' -> ')}`);
      return;
    }
...
export function validateCurriculumDataset(dataset: CurriculumDataset): string[] {
  ...
  for (const objective of dataset.objectives) {
    if (!topicIds.has(objective.topicId)) {
      errors.push(`Objective ${objective.id} references unknown topic: ${objective.topicId}`);
    }
    if (!nodeIds.has(objective.levelId)) {
      errors.push(`Objective ${objective.id} references unknown level: ${objective.levelId}`);
    }
  ...
  for (const representation of dataset.representations) {
    errors.push(...validateSourceRefs(`Representation ${representation.id}`, representation.sourceRefs, sourceIds));
  }
  for (const strategy of dataset.strategies) {
    errors.push(...validateSourceRefs(`Strategy ${strategy.id}`, strategy.sourceRefs, sourceIds));
  }
  for (const misconception of dataset.misconceptions) {
    errors.push(...validateSourceRefs(`Misconception ${misconception.id}`, misconception.sourceRefs, sourceIds));
  }

  for (const reference of dataset.textbookReferences) {
    if (!sourceIds.has(reference.sourceId)) {
      errors.push(`Textbook reference ${reference.id} references unknown source: ${reference.sourceId}`);
    }
    for (const id of reference.objectiveIds) {
      if (!objectiveIds.has(id)) {
        errors.push(`Textbook reference ${reference.id} references unknown objective: ${id}`);
      }
    }
  }
...
export function assertValidCurriculumDataset(dataset: CurriculumDataset): void {
  ...
  if (errors.length > 0) {
    throw new Error(`Invalid curriculum dataset:\n${errors.join('\n')}`);
  }
}
